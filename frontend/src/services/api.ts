...
export const fetchPublicMarketSummary = async (params: { symbols?: string[]; limit?: number } = {}) => {
  const { symbols, limit } = params;
  const { data } = await http.get<MarketSummaryResponse>('/api/public/market/summary', {
    params: {
      symbols: symbols?.length ? symbols.join(',') : undefined,
      limit,
    },
...
export const fetchPublicNews = async (
  ...
  const { currency = 'BTC', limit = 20 } = params;
  const { data } = await http.get<NewsResponse>('/api/public/news', {
    params: { currency, limit },
  });
  return data;
};
...
export interface StatsOverview {
  ...
  trend: Array<{ day: string; win_rate: number; total: number }>;
...
export interface TimeframePerformanceRow {
  timeframe: string;
  total_signals: number;
  win_count: number;
  loss_count: number;
...
export interface ConfidencePerformanceRow {
  bucket: string;
  total_signals: number;
  win_count: number;
  loss_count: number;
...
export interface RecentOutcomeRow {
  ...
  outcome: 'win' | 'loss' | 'breakeven' | 'pending' | string;
  checked_at?: string;
  created_at: string;
}

export const fetchStatsOverview = async (period: '7d' | '30d' | 'all' = '30d') => {
  const { data } = await http.get<StatsOverview>('/api/stats/overview', { params: { period } });
  return data;
};

export const fetchStatsByPair = async () => {
  const { data } = await http.get<{ items: PairPerformanceRow[] }>('/api/stats/by-pair');
  return data.items ?? [];
};

export const fetchStatsByTimeframe = async () => {
  const { data } = await http.get<{ items: TimeframePerformanceRow[] }>('/api/stats/by-timeframe');
  return data.items ?? [];
};

export const fetchStatsByConfidence = async () => {
  const { data } = await http.get<{ items: ConfidencePerformanceRow[] }>('/api/stats/by-confidence');
  return data.items ?? [];
};

export const fetchRecentOutcomes = async (limit = 20) => {
  const { data } = await http.get<{ items: RecentOutcomeRow[] }>('/api/stats/recent-outcomes', { params: { limit } });
  return data.items ?? [];
};
