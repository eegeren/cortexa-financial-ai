...
const MarketsDrawer = ({ open, items, activeSymbol, onClose, onSelectSymbol }: MarketsDrawerProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-sm">
      <div className="ml-auto flex h-full w-full flex-col overflow-hidden border-l border-white/10 bg-[#07101d] shadow-[-24px_0_80px_rgba(2,6,23,0.55)] sm:max-w-xl md:max-w-2xl lg:max-w-4xl">
            ...
            {filtered.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`grid w-full grid-cols-[1fr_1fr] items-center gap-2 rounded-2xl border px-3 py-3 text-left transition sm:grid-cols-[1.2fr_1fr_0.8fr_1fr] sm:gap-3 sm:px-4 ${
                    isActive
                ...
                >
                  ...
                  <p className="hidden text-sm text-slate-400 sm:block">{formatVolume(item.quote_volume ?? item.volume)}</p>
                </button>
              );
            })}
            {!filtered.length && (
