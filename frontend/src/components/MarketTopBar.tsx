...
const MarketTopBar = ({ items, loading = false, activeSymbol, onSelectSymbol, onViewAll }: MarketTopBarProps) => {
  const rows = items;

  return (
    <div className="sticky top-20 z-20 rounded-3xl border border-white/10 bg-slate-950/78 px-4 py-3 shadow-[0_18px_50px_rgba(2,6,23,0.28)] backdrop-blur-md">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-[11px] uppercase tracking-[0.3em] text-slate-500">Market Bar</p>
        </div>
        <button
          type="button"
          onClick={onViewAll}
      ...
      <div className="mt-3 flex gap-2 overflow-x-auto pb-1 scrollbar-none">
        {loading
          ? Array.from({ length: 5 }).map((_, index) => (
              <div key={index} className="h-16 min-w-[172px] animate-pulse rounded-2xl border border-outline/20 bg-slate-900/45" />
            ))
          : rows.map((item) => {
              const change = formatChange(item.price_change_percent);
              const tone =
              ...
              return (
                <button
                  ...
                  className={`min-w-[172px] rounded-2xl border px-3 py-3 text-left transition ${
                    isActive
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      {change && <span className={`text-xs font-medium ${tone}`}>{change}</span>}
                    </div>
                  </div>
