...
const BillingPage = () => {
  ...
  useEffect(() => {
    // Always refresh after a successful checkout so the new subscription shows
    if (checkoutSuccess) {
      void hydrate();
  ...
  }, [checkoutSuccess, hydrate, loadInvoices, loadProfile, subscription]);
    ...
    <div className="space-y-8 sm:space-y-12 lg:space-y-16">
      {checkoutSuccess && (
        <div className="rounded-2xl border border-emerald-400/25 bg-emerald-500/10 px-5 py-4 text-sm text-emerald-200">
          <strong className="font-semibold">Premium activated!</strong> Your subscription is now active. Welcome to the full experience.
        </div>
      )}
      ...
      {!canAccess && (
        <Paywall
          title="Plan inactive"
          description="Your AI access is paused. Upgrade to Premium ($15.99/mo) to unlock the full signal layer, AI assistant, and all markets."
