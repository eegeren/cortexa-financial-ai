...
func (s *AuthService) GoogleLogin(ctx context.Context, idToken string) (models.User, error) {
	resp, err := http.Get("https://oauth2.googleapis.com/tokeninfo?id_token=" + idToken)
	if err != nil {
		return models.User{}, fmt.Errorf("google token verification failed")
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return models.User{}, fmt.Errorf("invalid google id_token")
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&info); err != nil {
		return models.User{}, fmt.Errorf("invalid google token response")
	}

	if s.cfg.GoogleClientID != "" && info.Aud != s.cfg.GoogleClientID {
		return models.User{}, fmt.Errorf("google token audience mismatch")
	}
	if strings.ToLower(info.EmailVerified) != "true" {
		return models.User{}, fmt.Errorf("google email not verified")
	}
	if info.Email == "" {
		return models.User{}, fmt.Errorf("no email in google token")
	}

	// If no first/last name from Google, split the full name
	...
	var u models.User
	err = s.db.QueryRowContext(ctx, `
		INSERT INTO users(email, password_hash, role, created_at, first_name, last_name, kvkk_accepted, kvkk_accepted_at)
		VALUES($1, '', 'user', NOW(), $2, $3, true, NOW())
		ON CONFLICT (email) DO UPDATE
		  SET first_name = CASE WHEN users.first_name = '' THEN EXCLUDED.first_name ELSE users.first_name END,
		      last_name  = CASE WHEN users.last_name  = '' THEN EXCLUDED.last_name  ELSE users.last_name  END
		RETURNING id, email, role, first_name, last_name
	`, info.Email, firstName, lastName).Scan(&u.ID, &u.Email, &u.Role, &u.FirstName, &u.LastName)
	if err != nil {
		return models.User{}, err
	}

	if s.cfg.IsOwnerEmail(u.Email) && u.Role != "premium" {
		if _, dbErr := s.db.ExecContext(ctx, `UPDATE users SET role='premium' WHERE id=$1`, u.ID); dbErr == nil {
			u.Role = "premium"
		}
	}
	return u, nil
