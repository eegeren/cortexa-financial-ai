...
func computeSignalBreakdown(data *predictData) signalScoreBreakdown {
...
func signalFromPredictData(data *predictData) models.Signal {
	breakdown := computeSignalBreakdown(data)
...
func (s *SignalService) Predict(ctx context.Context, symbol, timeframe string) (models.Signal, error) {
	body, _ := json.Marshal(predictReq{Symbol: symbol, Timeframe: timeframe})
	predictURL := s.aiBaseURL() + "/predict"
	log.Printf("signal predict upstream url=%s symbol=%s timeframe=%s", predictURL, symbol, timeframe)
	...
	return signalFromPredictData(pr.Data), nil
}

func (s *SignalService) BatchPredict(ctx context.Context, symbols []string, timeframe string) ([]models.Signal, error) {
	if len(symbols) == 0 {
		return []models.Signal{}, nil
	}

	batchURL := s.aiBaseURL() + "/signals/batch"
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, batchURL, nil)
	q := req.URL.Query()
	...
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		return nil, fmt.Errorf("signal batch upstream request failed: %w", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		msg, _ := io.ReadAll(resp.Body)
	...
	var br batchPredictResp
	if err := json.NewDecoder(resp.Body).Decode(&br); err != nil {
		return nil, fmt.Errorf("ai service decode failed: %w", err)
	}
	if !br.OK {
		return nil, fmt.Errorf("ai service returned empty batch response")
	}

	results := make([]models.Signal, 0, len(br.Results))
	for _, item := range br.Results {
		if item.Data == nil {
			continue
		}
		results = append(results, signalFromPredictData(item.Data))
	}
