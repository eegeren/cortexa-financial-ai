...
func (s *BillingService) PaymentProvider() string {
	...
	switch provider {
	case "stripe", "paddle", "lemonsqueezy", "iyzico", "nowpayments", "demo":
		return provider
	default:
		return "stripe"
	}
...
func (s *BillingService) HasPremiumAccess(ctx context.Context, userID int64, role string) (bool, error) {
	...
	if email, err := s.lookupUserEmail(ctx, userID); err == nil && s.cfg.IsOwnerEmail(email) {
		return true, nil
	}
	...
	normalizedPlan := strings.ToLower(strings.TrimSpace(sub.PlanCode))
	return sub.Status == "active" && (normalizedPlan == "premium" || normalizedPlan == "pro" || normalizedPlan == "enterprise"), nil
}

func (s *BillingService) CreateCheckoutSession(ctx context.Context, userID int64, planCode, origin, successURL, cancelURL string) (models.CheckoutSession, error) {
	...
	if provider == "nowpayments" {
		if s.cfg.NowPaymentsPaymentURL != "" {
			checkoutURL = s.cfg.NowPaymentsPaymentURL
		} else {
			invoiceURL, err := s.createNowPaymentsInvoice(sessionID, plan, origin, successURL, cancelURL)
			if err != nil {
				return session, fmt.Errorf("nowpayments: %w", err)
			}
			checkoutURL = invoiceURL
		}
	} else {
		checkoutURL = s.buildCheckoutURL(provider, sessionID, plan.Code, origin, successURL, cancelURL)
	}
	providerCustomerID := fmt.Sprintf("cust-%d", userID)
...
func (s *BillingService) buildCheckoutURL(provider, sessionID, planCode, origin, successURL, cancelURL string) string {
	// Demo provider: activate subscription instantly via a backend redirect endpoint.
	if provider == "demo" {
		base := origin
		if base == "" {
			base = "http://localhost:8080"
		}
		return fmt.Sprintf("%s/api/billing/demo-activate?session_id=%s", base, sessionID)
	}
...
// createNowPaymentsInvoice calls the NOWPayments API to create a hosted invoice
// and returns the invoice URL the user should be redirected to.
func (s *BillingService) createNowPaymentsInvoice(orderID string, plan models.Plan, origin, successURL, cancelURL string) (string, error) {
	ipnURL := origin + "/api/webhooks/payment?provider=nowpayments"

	priceAmount := float64(plan.AmountCents) / 100.0
	body := map[string]any{
		"price_amount":      priceAmount,
		"price_currency":    strings.ToLower(plan.Currency),
		"order_id":          orderID,
	...
	req, err := http.NewRequest(http.MethodPost, "https://api.nowpayments.io/v1/invoice", bytes.NewReader(bodyBytes))
	if err != nil {
		return "", err
	}
	...
	if resp.StatusCode >= 300 {
		return "", fmt.Errorf("nowpayments API error %d: %s", resp.StatusCode, string(respBytes))
	}
	...
	if result.InvoiceURL == "" {
		return "", fmt.Errorf("nowpayments returned empty invoice_url")
	}
	return result.InvoiceURL, nil
}

// handleNowPaymentsWebhook verifies the IPN signature and activates the subscription.
func (s *BillingService) handleNowPaymentsWebhook(ctx context.Context, headers map[string]string, payload []byte) error {
	normalized := map[string]string{}
	for k, v := range headers {
		normalized[strings.ToLower(k)] = v
	}

	if secret := s.cfg.NowPaymentsIPNSecret; secret != "" {
		sig := normalized["x-nowpayments-sig"]
		if sig == "" {
			return fmt.Errorf("missing x-nowpayments-sig header")
		}
		sorted, err := sortedJSONBytes(payload)
		if err != nil {
	...
	if err := json.Unmarshal(payload, &ipn); err != nil {
		return fmt.Errorf("invalid nowpayments IPN: %w", err)
	}

	status := strings.ToLower(strings.TrimSpace(ipn.PaymentStatus))
	if status != "finished" && status != "confirmed" {
		// Not a terminal success state — acknowledge but do nothing
		return nil
	}

	orderID := strings.TrimSpace(ipn.OrderID)
	if orderID == "" {
		return fmt.Errorf("missing order_id in nowpayments IPN")
	}
	...
	if affected == 0 {
		return fmt.Errorf("nowpayments: no subscription found for order_id %s", orderID)
	}

	if _, err := tx.ExecContext(ctx, `
		UPDATE users SET role = 'premium'
		WHERE id = (SELECT user_id FROM subscriptions WHERE provider_subscription_id = $1)
	`, orderID); err != nil {
		return err
	}
...
// sortedJSONBytes unmarshals JSON and re-encodes it with keys sorted alphabetically.
// NOWPayments signature verification requires this canonical form.
func sortedJSONBytes(data []byte) ([]byte, error) {
...
// DemoActivate marks the pending subscription (identified by session_id) as active.
// Only works when PAYMENT_PROVIDER=demo.
func (s *BillingService) DemoActivate(ctx context.Context, sessionID string) error {
	if s.PaymentProvider() != "demo" {
		return fmt.Errorf("demo activation only available in demo mode")
	}
...
func (s *BillingService) HandleWebhook(ctx context.Context, provider string, headers map[string]string, payload []byte) error {
	...
	if provider == "nowpayments" {
		return s.handleNowPaymentsWebhook(ctx, headers, payload)
	}
