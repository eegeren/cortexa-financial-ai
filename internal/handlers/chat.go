...
func (h *Handlers) ChatCompletion(w http.ResponseWriter, r *http.Request) {
	...
	canAccess, _, err := h.Billing.CanAccessAssistant(r.Context(), userID)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	// Also allow by role (admin/premium JWT claim)
	normalizedRole := strings.ToLower(strings.TrimSpace(role))
	if !canAccess && normalizedRole != "admin" && normalizedRole != "premium" {
