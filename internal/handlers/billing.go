...
func (h *Handlers) BillingCheckout(w http.ResponseWriter, r *http.Request) {
	...
	session, err := h.Billing.CreateCheckoutSession(r.Context(), userID, planCode, origin, req.SuccessURL, req.CancelURL)
	if err != nil {
...
// BillingDemoActivate is a public GET endpoint that activates the subscription
// associated with the given session_id and then redirects the user to /billing.
// Only functional when PAYMENT_PROVIDER=demo.
func (h *Handlers) BillingDemoActivate(w http.ResponseWriter, r *http.Request) {
	if h.Billing == nil {
		http.Error(w, "billing unavailable", http.StatusServiceUnavailable)
		return
	}
	...
	if err := h.Billing.DemoActivate(r.Context(), sessionID); err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
	// Redirect to billing page with success indicator
	http.Redirect(w, r, "/billing?checkout=success", http.StatusSeeOther)
}
